#!/usr/bin/env bash
# Installer for cron or systemd timer and making scripts executable
set -Eeuo pipefail
...
print_menu() {
  cat <<'EOF'
Installer Options
1) Install sample crontab (root)
2) Install systemd timer for log scanning (root)
...
main() {
  ...
  case "$ans" in
    ...
    2)
      ...
      cat > "$svc_path" <<EOF
[Unit]
Description=Bash Maintenance Suite - Log Scan
